...
$auth_users = array(
    'admin' => '$2y$10$/K.hjNr84lLNDt8fTXjoI.DBp6PpeyoJ.mGwrrLuCZfAwfSAGqhOW', //admin@123 (Used only if SSO fails)
);
...
if (defined('FM_EMBED')) {
    ...
    // RADAR SSO BRIDGE v2.1: Read Radar session FIRST (PHP default session)
    // This is needed because XTool uses a different session name ('filemanager')
    @session_start();
    $radarAuth = $_SESSION['radarkit_auth'] ?? null;
    session_write_close(); // Close default session before switching

    // Now switch to XTool's own session
    ...
    // RADAR SSO BRIDGE v2.1: Auto-login if Radar session was valid
    // Only set login if not already logged in (avoid overwriting existing session)
    if ($radarAuth && $radarAuth['authorized'] === true && $radarAuth['ip'] === $_SERVER['REMOTE_ADDR']) {
        if (!isset($_SESSION[FM_SESSION_ID]['logged'])) {
            $_SESSION[FM_SESSION_ID]['logged'] = 'admin';
            // Force regenerate CSRF token for new SSO session
            ...
            <div class="card-body">
                <form id="js-settings-form" action="" method="post" data-type="ajax" onsubmit="return save_settings(this)">
                    ...
                    <small class="text-body-secondary">*
                        <?php echo lng('Sometimes the save action may not work on the first try, so please attempt it again') ?>.</small>
                </form>
            </div>
        </div>
    </div>
